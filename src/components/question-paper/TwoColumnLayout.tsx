...
export function TwoColumnLayout({ children, className }: TwoColumnLayoutProps) {
  ...
  return (
    <div
      ref={containerRef}
      className={cn("two-column-layout", "h-full relative", className)}
      style={{
        // CSS columns 기본 설정
        columnCount: 2,
        columnGap: "20mm",
...
export function ColumnBreak({ children, breakBefore = false, breakAfter = false, breakInside = "auto" }: ColumnBreakProps) {
  return (
    <div
      style={{
...
export function SmartTwoColumnLayout({
  ...
  // 문제 그룹의 예상 높이 계산
  const estimateGroupHeight = useCallback(
    (group: QuestionGroupType): number => {
      let height = 50; // 기본 여백
  ...
  return (
    <div
      className={cn("smart-two-column-layout", "grid grid-cols-2 h-full", className)}
      style={{
      ...
      >
        <div className="space-y-0">
          {leftGroups.map((group) => (
            <div key={group.id} data-group-id={group.id} className="break-inside-avoid">
 ...
 */
export function ManualTwoColumnLayout({ leftColumn, rightColumn, className }: ManualTwoColumnLayoutProps) {
  return (
    <div
      className={cn("manual-two-column-layout", "grid grid-cols-2 gap-0 h-full", className)}
      style={{
        gridTemplateColumns: "1fr 1fr",
        columnGap: "0",
      }}
